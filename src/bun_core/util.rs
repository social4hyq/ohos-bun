...
/// Like [`getcwd`], but on OHOS surfaces a deleted cwd as
/// [`crate::CrateError::CurrentWorkingDirectoryUnlinked`] instead of the
/// compat shim's silent `$HOME` substitution; for callers that must
...
/// OHOS-only deleted-cwd re-check bypassing the compat shim's silent `$HOME`
/// substitution in plain `getcwd()`; mirrors `bun_sys::posix::cwd_is_deleted`
/// (duplicated because `bun_sys` depends on `bun_core`).
#[cfg(target_env = "ohos")]
...
pub fn getcwd_or_exe_dir(buf: &mut PathBuffer) -> &ZStr {
    ...
    // OHOS: the compat shim's getcwd() silently substitutes $HOME for a
    // deleted cwd, so re-check /proc/self/cwd here and fall through to the
    // exe-dir fallback if it's really gone.
    #[cfg(target_env = "ohos")]
