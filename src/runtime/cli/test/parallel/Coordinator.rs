...
impl<'a> Coordinator<'a> {
    ...
    fn find_steal_victim(&mut self) -> Option<*mut Worker> {
        ...
        for i in 0..len {
            ...
            // Skip spawned-but-not-Ready workers (alive, no inflight): OHOS's slow fork lets a fast worker steal a just-spawned sibling's range, which it re-steals on Ready — running files under the thief's worker ID; alive=false is an unclaimed reservation, fine to steal.
