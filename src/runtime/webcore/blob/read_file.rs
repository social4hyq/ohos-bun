    ...
    fn do_read_loop(&mut self) {
        #[cfg(not(windows))]
        {
            ...
            while self.state.load(Ordering::Relaxed) == ClosingState::Running as u8 {
                ...
                // already delivered everything past `end` (`offset`/`start`
                // was unaffected — it's applied via `set_file_offset`/lseek in
                // `resolve_size_and_last_modified`, a separate code path).
                #[cfg(target_env = "ohos")]
                let (buf_ptr, buf_len) = {
