    ...
    fn thread_main(manager: &'static PathWatcherManager) {
        ...
        while running.load(Ordering::Acquire) {
            ...
            while i < n {
                ...
                // Suppress OHOS's creation-labeling ATTRIB (see
                // `attrib_shadowed_by_create`); `i` already points past this
                // event, at the first lookahead candidate.
                if ev.mask & IN::ATTRIB != 0 && !name.is_empty() {
                    let attrib_wd = ev.watch_descriptor;
                    let mut shadowed = attrib_shadowed_by_create(&buf.0[..n], i, attrib_wd, name);
                    if !shadowed && n < buf.0.len() {
                        // The labeling CREATE is queued by the same syscall
                        // but can land after we wake on ATTRIB, so same-batch
                        // lookahead alone races: poll briefly and re-check
                        // across the read boundary. Newly read events extend
                        // this batch (`n` grows) so ordering is intact, and
                        // the re-read only writes the tail `buf.0[n..]`,
                        // disjoint from `ev`/`name`.
...
/// OHOS kernels queue a spurious IN_ATTRIB (security labeling) immediately
/// before IN_CREATE on open(O_CREAT)/mkdir, so the reader suppresses an
/// ATTRIB when a later event in the same read buffer is an IN_CREATE for the
/// same (wd, name); genuine attribute changes (chmod/chown/utimens) are
/// never followed by IN_CREATE and pass through. The small lookahead bound
/// covers interleaving from other writers without an O(n^2) scan.
#[cfg(any(target_os = "linux", target_os = "android"))]
