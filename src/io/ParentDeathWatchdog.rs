...
fn list_child_pids_linux(parent: libc::pid_t, out: &mut [libc::pid_t]) -> Option<usize> {
    ...
    // Distinguish "no children" (file present, empty) from "kernel lacks
    // CONFIG_PROC_CHILDREN" (file absent) — the latter must not be reported
    // as a successful empty enumeration.
...
/// Fallback for kernels without `CONFIG_PROC_CHILDREN` (e.g. OpenHarmony):
/// scan `/proc` and keep entries whose `stat` ppid is `parent`. O(processes)
/// per call, so teardown path only.
#[cfg(any(target_os = "linux", target_os = "android"))]
